...
def test_checksum_ok_true_when_checksum_unset(external_root):
    """外部 turbo-only 形态（skill_codes_dir 空）下未配置 checksum -> True。

    修复前：校验块挂在 _scan_skills_dir 末尾，被空 skill_codes_dir 的
    提前 return 短路，_skill_checksum_ok 恒为初始 False 注入每次执行
    inputs（与「未配置视为通过」的旧语义漂移）。
    """
    env = _make_env()
    assert env.skill_checksum_ok is True


def test_checksum_ok_matches_configured_checksum(external_root):
    from jiuwenswarm.server.runtime.skill_turbo.environment import (
        _compute_dir_checksum,
    )

    skill_dir = external_root / "fake-check-craft"
    skill_dir.mkdir()
    (skill_dir / "SKILL.md").write_text("body", encoding="utf-8")
    expected = _compute_dir_checksum(str(skill_dir))

    env = _make_env(
        skills_dir=str(external_root),
        skill_name="fake-check-craft",
        skill_checksum=expected,
    )
    assert env.skill_checksum_ok is True


def test_checksum_ok_false_on_mismatch(external_root):
    skill_dir = external_root / "fake-check-craft"
    skill_dir.mkdir()
    (skill_dir / "SKILL.md").write_text("body", encoding="utf-8")
    ...
    assert env.skill_checksum_ok is False


def test_checksum_ok_true_when_skill_root_unresolved(monkeypatch):
    import jiuwenswarm.common.utils as jw_utils

    monkeypatch.setattr(jw_utils, "resolve_agent_registered_skill_dirs", lambda: [])
    # skills_dir 指向不存在目录 -> 两级解析都失败 -> skill_root 为空 -> True
    env = _make_env(skills_dir=str(Path(__file__).parent / "no-such-dir"))
    assert env.skill_root == ""
    assert env.skill_checksum_ok is True

...
`build_member_capability_specs(config, mode, role) -> (rails, tools)` 按 mode 分档；`build_member_subagent_specs` 仅 code 档（code/browser 受 `react.subagents.<name>.enabled` 门控）；`build_member_deep_agent_spec` 把能力 spec 折叠到 base `DeepAgentSpec`。
...
- Sub-agent：`core.browser_agent`（language / max_iterations 为 params，model 取 `ctx.extras["_parent_model"]`）。`core.explore_agent` / `core.plan_agent` provider 仍保留注册，当前临时版本不装配。

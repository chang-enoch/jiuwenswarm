...
- 面向**用户输出**时不回显原文；面向**工具调用与执行**时仍按第 6 条原样使用 SKILL.md 的参数值与标签，二者不冲突。
...
- When addressing the **user**, do not echo the source text; when making **tool calls / executing**, still use SKILL.md parameter values and labels verbatim per rule 6 — the two do not conflict.

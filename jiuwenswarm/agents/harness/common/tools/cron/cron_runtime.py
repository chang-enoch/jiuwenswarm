...
class _CronToolsCronBackend(CronToolBackend):
    ...
    async def create_job(
        ...
        *,
        ...
        # 现场。任务描述常含"每天/每周…"等字样，模型若还能调到 cron 工具的
        # add（注册守卫失效或历史消息残留调用）会把调度再建一遍，这里直接拒绝，
        # 把模型引回"完成当前任务内容"。
...
def _extract_legacy_params(
    ...
    *,
    ...
    if "schedule" in data or "payload" in data or "delivery" in data:
        ...
        if require_schedule:
            # create 一律建启用任务：enabled 只在 update/toggle 中有意义。
            # 显式 false 或 null 都不再产生禁用任务（null 走 bool() 会变成 False）。
            out["enabled"] = True
    ...
    if require_schedule:
        ...
        # create 一律建启用任务（与嵌套分支一致）：显式 false/null 不再产生
        # 禁用任务，用户建好即生效；停用走 update/toggle 显式操作。
        data["enabled"] = True
...
class CronRuntimeBridge:
    ...
    def build_tools(self, *, context: Any, agent_id: Optional[str], language: str = "cn") -> list[Any]:
        """Build the unified cron tool (single model-facing entry)."""
        backend = self.get_backend()
        if backend is None:

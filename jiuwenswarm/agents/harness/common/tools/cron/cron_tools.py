...
class CronTools:
    ...
    async def create_job(self, params: dict[str, Any]) -> Any:
        ...
        job = await self._local_store.create_job(
            ...
            targets=targets_str,
            # 建好即启用：None/缺省/显式 true 一律 True，只有显式 false 才建禁用
            # 任务（None 若走 bool() 会静默变成 False，见 create 链路兜底）。
            enabled=bool(normalized.get("enabled")) if normalized.get("enabled") is not None else True,

...
_EXTENSIONS = 'pptx ppt potx docx doc odt xlsx xls xlsm csv tsv pdf png jpg jpeg svg webp html md txt json mp3 wav mp4'.split()
_TOKEN = re.compile(
...
_ACTIONS = {
    'create': r'创建|生成|制作|新建|起草|编写|撰写|导出|输出|交付|保存为|另存为|整理成|做成|转换为|转换成|转为|转成|'
              r'\b(?:creat(?:e|es|ing|ion)|generat(?:e|es|ing|ion)|make|build|write|draft|export(?:ing)?|deliver(?:ing)?|output|produce|save(?:d)? as)\b',
    'edit': r'(?<!可)(?<!可以)编辑|修改|更新|重写|改写|替换|纠正|修正|润色|增删|精简|缩减|删减|扩写|\b(?:edit(?:ing)?|modify|modifying|update|revise|rewrite|correct|fix)\b',
    'read': r'读取|阅读|提取|抽取|解析|总结|摘要|分析|\b(?:read(?:ing)?|pars(?:e|ing)|extract(?:ing)?|summari[sz](?:e|ing)|inspect)\b',
...
_CONVERT = re.compile(r'\b(?:convert|turn|export|save)\b.*?\b(?:to|into|as)\b|(?:转换|导出|保存|另存|转)(?:成|为)', re.I)
_SPLIT = re.compile(r'[。！？!?;；\n]|\.(?=\s|$)')
_MACRO_WRITE = re.compile(r'(?:编写|生成|创建|制作|\b(?:write|create|generate|build)\b).{0,80}?(?:宏|\b(?:macros?|vba)\b)', re.I)
...
def _bindings(clause, *, declarations=False):
    ...
    for i, (mention, values) in enumerate(formats):
        ...
        before = [a for a in actions if previous_end <= a.start() < mention.start()
                  and mention.start()-a.end() <= 110]
        operations = ()
        if before:
    ...
    for raw in (description, *sources):
        ...
        for clause in clauses:
            ...
            if any('xlsm' in values for _,values in _formats(clause, declarations=True)):
                if _MACRO_WRITE.search(clause): target.add(('create_macros', 'xlsm'))
                if _MACRO_RUN.search(clause): target.add(('run_macros', 'xlsm'))
            ...
            if _OUTPUT_LABEL.search(clause) and not _INPUT.search(clause):
                ...
                if not negative:
                    outputs.update(fs)
                    exclusive = bool(fs and re.search(r'仅|只有|只(?:能)?|\bonly\b|所有输出.*必须|all outputs.*must', clause, re.I))
                    if exclusive:
                        closed = True
                    ...
                    if re.search(r'presentations?|slides?|decks?|workbooks?|spreadsheets?|工作簿|表格', clause, re.I):
                        for action in _ACTION.finditer(clause):
                            for f in fs:
                                ...
                                if action.lastgroup == 'create': outputs.add(f)
    if closed:

...
CASES = {
    ...
    'excel': '请生成一个 Excel 工作簿，录入销售额：1 月 12000 元、2 月 15000 元、3 月 18000 元。使用公式计算合计和平均值，并生成柱状图，交付 .xlsx 文件。',
    'word': '请将以下内容整理成正式的会议纪要，交付 .docx 文件：今天讨论了官网改版，决定先改首页和产品介绍页；小王负责设计，下周三交初稿；小李负责开发，下周五完成；预算尚未确定，需要负责人确认。请列出决议、待办事项和待确认问题。',
}
...
QUERIES = {
    ...
    'excel': ('生成销售额 Excel 工作簿，公式计算合计平均值并生成柱状图，交付 xlsx', ['excel', 'xlsx', 'spreadsheet', 'formula', 'chart']),
...
async def measure(args):
    ...
    started = time.perf_counter()
    rail.init(agent)
    pipeline, _ = await rail._service.current_snapshot()
    ...
    report = {'measurement': 'offline; no LLM, no service, no skill execution',
              'warm_scope': 'search tool with fixed query fixtures; excludes model generation, SDK rails and skill loading',
    ...
    try:
        for name, query in CASES.items():
            ...
            for _ in range(args.samples):
                ...
                result = await rail._from_model('search', ctx, query=model_query, keywords=keywords)
        ...
        large = SelectionPipeline(rail._service.settings)

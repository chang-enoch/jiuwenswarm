...
| `react.context_engine_config.dialogue_compressor_config.trigger_token_threshold` | 对话压缩绝对 Token 触发阈值（填了优先于比例） | 无（留空） |
| `react.context_engine_config.dialogue_compressor_config.target_retention_ratio` | 对话压缩目标保留率（压缩后 ≈ 被压缩内容 × 该比例） | 无（留空） |
